...
public class RecipeDatabase : MonoBehaviour
{
    ...
    /// </summary>
    public List<ProcessingRecipe> GetAvailableRecipes(ItemData item, ProcessingState itemState, ToolState tool)
    {
        ...
        foreach (var recipe in allRecipes)
        {
            ...
            if (recipe.inputItem == item && 
                recipe.requiredInputState == itemState &&
    ...
    /// </summary>
    public ProcessingRecipe FindRecipe(ActionType action, ToolType tool, ItemData item = null, ProcessingState itemState = ProcessingState.Raw)
    {
        return allRecipes.FirstOrDefault(r => 
            r.actionType == action && 
            r.requiredTool == tool &&
            (item == null || r.inputItem == item) &&
            (item == null || r.requiredInputState == itemState)
        );
    }
    ...
    /// </summary>
    public bool IsActionAvailable(ItemData item, ProcessingState itemState, ToolState tool, ActionType action)
    {
        var availableRecipes = GetAvailableRecipes(item, itemState, tool);
